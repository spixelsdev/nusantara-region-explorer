...
<template>
  <div class="space-y-4">
    ...
    <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-3 pt-2">
      <div>
        <h2 class="text-lg font-bold text-slate-900 dark:text-slate-100 flex items-center gap-2">
          <MapPin class="w-4 h-4 text-sky-600 dark:text-sky-400" />
    ...
    <!-- Loading State -->
    <div v-if="store.loading" class="flex flex-col items-center justify-center py-16 text-slate-400 gap-2 border border-dashed border-slate-200 dark:border-slate-800 rounded-md">
      <Loader2 class="w-6 h-6 animate-spin text-sky-600 dark:text-sky-400" />
      <span class="text-xs">Mengambil data dari endpoint...</span>
    </div>

    <!-- Error State -->
    <div v-else-if="store.error" class="p-4 border border-rose-200 dark:border-rose-900 bg-rose-50 dark:bg-rose-950/40 text-rose-700 dark:text-rose-300 rounded-md text-xs">
      {{ store.error }}
    </div>

    <!-- Empty State -->
    <div v-else-if="store.filteredList.length === 0" class="py-12 text-center text-slate-400 border border-slate-200 dark:border-slate-800 rounded-md text-xs">
    ...
    <div v-else class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-2.5">
      ...
      >
        ...
        <div class="shrink-0 text-slate-400 group-hover:text-slate-700 dark:group-hover:text-slate-200">
